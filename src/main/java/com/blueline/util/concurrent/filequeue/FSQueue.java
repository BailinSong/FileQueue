...
public class FSQueue {
	 ...
	 */
	private LogEntity createLogEntity(String dbpath, LogIndex db, long fileNumber) throws IOException,
			FileFormatException {
	...
	public byte[] readNextAndRemove() throws IOException, FileFormatException {
		...
		try {
			...
			long deleteNum = readerHandle.getCurrentFileNumber();
			long nextfile = readerHandle.getNextFile();
			readerHandle.close();
